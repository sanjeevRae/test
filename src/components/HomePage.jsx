...
const HomePage = () => {  const location = useLocation();
  ...
  const [billingCycle, setBillingCycle] = useState('monthly');  // Five Cards Animation
  useEffect(() => {
    const handleScroll = () => {
      if (fiveCardsRef.current) {
        const fiveCardsSection = fiveCardsRef.current;
        const rect = fiveCardsSection.getBoundingClientRect();
        const windowHeight = window.innerHeight;
        
        // Check if the section is in view
        const isInView = rect.top < windowHeight * 0.7 && rect.bottom > windowHeight * 0.3;
        
        // Check if we're near the end of the section for end animation
        const isNearEnd = rect.top < windowHeight * 0.3 && rect.bottom > windowHeight * 0.1;
        
        if (isInView) {
          fiveCardsSection.classList.add('spread');
          
          // Add end animation when near the end of the section
          if (isNearEnd) {
            fiveCardsSection.classList.add('end-animation');
            
            // Add pulse animation after a short delay
            setTimeout(() => {
              if (fiveCardsSection.classList.contains('end-animation')) {
                fiveCardsSection.classList.add('pulse');
              }
            }, 500);          } else {
            fiveCardsSection.classList.remove('end-animation', 'pulse');
          }
        } else {
          fiveCardsSection.classList.remove('spread', 'end-animation', 'pulse');
        }
      }
    };
      ...
      <section id="about" className="about-section workforce-hero" ref={aboutRef}>
        <div className="container">
          <div className="workforce-content">
           ...
           {/* Five-section preview with left-center-right layout */}
         <section className="five-preview-container" id="five-preview" ref={fiveCardsRef}>
           <div className="stacked-cards">
             {/* Left Column */}             <div className="preview-section section-left-1">
               <div className="section-content">
                 <img src ="/src/assets/leftA.jpg" alt="Left section 1" className="section-image"/>
               </div>
             </div>
             
             <div className="preview-section section-left-2">
               <div className="section-content">
                 <img src="/src/assets/leftB.jpg" alt="Left section 2"className="section-image"/>
               </div>
             </div>
             
             {/* Center Column - Taller */}
             <div className="preview-section section-center">
               <div className="section-content">
                 <img src="/src/assets/Mid.png" alt="Center section" className="section-image"/>
               </div>
             </div>
             
             {/* Right Column */}
             <div className="preview-section section-right-1">
               <div className="section-content">
                 <img src="/src/assets/rightA.gif" alt="Right section 1" className="section-image" />
               </div>
             </div>
             
             <div className="preview-section section-right-2">
               <div className="section-content">
                 <img src="/src/assets/rightB.jpg"  alt="Right section 2" className="section-image"/>
               </div>
             </div>
           </div>
         </section>
