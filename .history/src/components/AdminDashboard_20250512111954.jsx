          ...
          <tbody>
            {users.map(user => (
              <tr key={user.id}>
                ...
                <td>
                  <select value={user.subscriptionExpires ? 'active' : 'none'} onChange={e => handleSubscriptionChange(user.id, e.target.value)}>
                    <option value="none">No Subscription</option>
                    <option value="1">1 Month</option>
                    <option value="3">3 Months</option>
                    <option value="12">12 Months</option>
                  </select>
                  {user.subscriptionExpires && <div style={{fontSize:'0.85em'}}>Exp: {new Date(user.subscriptionExpires).toLocaleDateString()}</div>}
                </td>
                <td>
