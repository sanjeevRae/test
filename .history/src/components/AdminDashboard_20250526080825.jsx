...
const AdminDashboard = () => {
  ...
  const [authorized, setAuthorized] = useState(false);  const [activeTab, setActiveTab] = useState('all'); // all, active, suspended, expired
  const [activeSection, setActiveSection] = useState('users'); // users, organizations - start with users view
  ...
  const handleRoleChange = async (uid, role) => {
    try {
      ...
      // Get the organizations for this admin to assign to
      const orgsCol = collection(db, 'organizations');
      const orgsSnapshot = await getDocs(orgsCol);
      
      let organizationId = null;
      
      // If role is 'leader', we need to assign an organization
      if (role === 'leader') {
        // Get list of organizations to show in prompt
        ...
        // Format organizations for display in prompt
      ...
      // Update user role using setUserRole from auth.js
      ...
      if (success) {
        ...
        // Get the updated user data from Firestore to verify
        ...
        if (updatedUserDoc.exists()) {
          ...
          // If the role in the database doesn't match the UI, refresh the whole list
          if (updatedRole !== role) {
  ...
  return (
    <div className="admin-dashboard">      <div className="admin-header">
        ...
        <p className="admin-subtitle">Manage users, organizations, subscriptions and access control</p>
