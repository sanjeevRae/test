...
const HomePage = () => {
  ...
  const sevenCardsRef = useRef(null);
  const [activeTab, setActiveTab] = useState('team');
  const [billingCycle, setBillingCycle] = useState('monthly');

  // Seven Cards Animation
  useEffect(() => {
    const handleScroll = () => {
      if (sevenCardsRef.current) {
        const sevenCardsSection = sevenCardsRef.current;
        const rect = sevenCardsSection.getBoundingClientRect();
        ...
        if (isInView) {
          sevenCardsSection.classList.add('spread');
          
          // Add end animation when near the end of the section
          if (isNearEnd) {
            sevenCardsSection.classList.add('end-animation');
            
            // Add pulse animation after a short delay
            setTimeout(() => {
              if (sevenCardsSection.classList.contains('end-animation')) {
                sevenCardsSection.classList.add('pulse');
              }
            }, 500);
          } else {
            sevenCardsSection.classList.remove('end-animation', 'pulse');
          }
        } else {
          sevenCardsSection.classList.remove('spread', 'end-animation', 'pulse');
        }
      }
    };
    ...
    <div className="home-page">
      ...
      <section id="about" className="about-section workforce-hero" ref={aboutRef}>
        <div className="container">
         ...
         <section className="seven-preview-container" id="seven-preview" ref={sevenCardsRef}>
