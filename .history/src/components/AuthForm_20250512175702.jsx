...
const AuthForm = ({ isSignup = false }) => {
  ...
  const handleSignIn = async () => {
    try {
      ...
      try {
        ...
        // Check if the email is admin@evox.com (case insensitive)
        ...
        if (!userDoc.exists()) {
          ...
          console.log('New user document created, redirecting to:', isAdmin ? '/admin' : '/dashboard');
          if (isAdmin) {
            navigate('/admin');
          } else {
            navigate('/dashboard');
          ...
          // Force admin role if email is admin@evox.com
        ...
        // Check if the email is admin@evox.com as a fallback
        if (email.toLowerCase() === 'admin@evox.com') {
          console.log('Firestore failed but user is admin, redirecting to admin page');
          navigate('/admin');
        } else {
          navigate('/dashboard');
        }
  ...
  const handleSignUp = async () => {
    try {
      ...
      // Determine if this is the admin account (case insensitive)
      const isAdmin = email.toLowerCase() === 'admin@evox.com';
      console.log('Is admin signup?', isAdmin);
      ...
      try {
        ...
        if (isAdmin) {
          ...
          console.log('Regular user account created, redirecting to dashboard');
          navigate('/dashboard');
        }
      } catch (firestoreError) {
        ...
        // Check if admin for redirection, even if Firestore fails
        if (isAdmin) {
          navigate('/admin');
        } else {
          navigate('/dashboard');
