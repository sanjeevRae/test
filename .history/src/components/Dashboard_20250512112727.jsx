...
const Dashboard = () => {
    ...
    if (user) {
      const fetchProfile = async () => {
        ...
        if (docSnap.exists()) {
          ...
          // Fetch analytics for premium/executive users
          if (['Premium', 'Executive'].includes(data.category)) {
            ...
            if (analyticsSnap.exists()) {
              ...
              setAnalytics({ views: 0, qrScans: 0 });
            }
          }
        }
      };
  ...
  // Track profile views in Firestore for premium/executive users
  useEffect(() => {
    if (!user) return;
    if (['Premium', 'Executive'].includes(profile.category)) {
      const incrementAnalytics = async () => {
        ...
        const analyticsSnap = await getDoc(analyticsRef);
        const prevViews = analyticsSnap.exists() ? analyticsSnap.data().views || 0 : 0;
        await setDoc(analyticsRef, { views: prevViews + 1 }, { merge: true });
        // Fetch updated analytics
        const updatedSnap = await getDoc(analyticsRef);
        if (updatedSnap.exists()) {
          setAnalytics(updatedSnap.data());
        }
      };
      incrementAnalytics();
    }
