...
// Import enhanced styles for better visibility
import './OrganizationManagementEnhanced.css';

const OrganizationManagement = () => {  const [organizations, setOrganizations] = useState([]);
  ...
  const loadOrganizations = async () => {
    ...
    try {
      ...
      // If there are organizations, select the first one automatically
    ...
    try {
      ...
      if (orgDetails) {
        ...
        // Log the members to check their roles
  ...
  const loadAvailableUsers = async () => {
    try {
      ...
      const availUsers = allUsers.filter(user => {
        return !user.organizationId || user.organizationId !== activeOrg?.id;
      });
    ...
    try {
      ...
      if (orgId) {
        ...
        // Explicitly select the newly created org
        ...
        alert('Failed to create organization. Please make sure you have admin permissions.');
      }
    } catch (error) {
    ...
    // Check if user is a leader - leaders cannot add new members
    if (userRole === 'leader') {
  ...
  const handleAddExistingUser = async () => {
    ...
    try {
      ...
      // Verify user exists before attempting to add them
      ...
      const success = await addUserToOrganization(
        selectedUserId,
        activeOrg.id,
        'user' // Default role for existing users
      );
    ...
    try {
      ...
      // Get the member from the local state to confirm we're operating on the right one
      ...
      // Verify that the user is valid before attempting to update UI
      ...
      // Make sure we have the correct permissions before updating
      const currentUserRole = await getUserRole();
      console.log(`Current user's role: ${currentUserRole}`);
      
      // Only update local state after we've verified the user exists
      setOrgMembers(prevMembers => 
        prevMembers.map(member => 
          member.id === userId ? {...member, role: newRole} : member
        )
      );      // Call setUserRole with improved error handling
      const result = await setUserRole(userId, newRole, activeOrg.id);
      
      // Use compatibility helper to determine success status
      if (getSuccessFromResult(result)) {
        console.log(`User role updated successfully to ${newRole}`);
        
        // Verify the change by force refreshing the organization data
        // This ensures both the users collection and org members collection are in sync
        setTimeout(() => {
          handleSelectOrg(activeOrg.id);
        }, 500);
      } else {
        // Get a meaningful error message using our helper
        ...
        // Refresh from server on error to restore the correct state
        await handleSelectOrg(activeOrg.id);
      }
    } catch (error) {
