  ...
  const checkUserPermissions = async () => {
    ...
    try {
      ...
      if (getSuccessFromResult(result)) {
        console.log(`User role updated successfully to ${newRole}`);        // If the current user is the one being updated, refresh their permissions
        if (userId === auth.currentUser?.uid) {
          console.log('Current user role changed, refreshing permissions...');
          // Add a delay to ensure Firebase has time to update
          setTimeout(async () => {
            await checkUserPermissions();
            console.log('User permissions refreshed after role change');
            // Reload the page to ensure all permissions are properly applied
