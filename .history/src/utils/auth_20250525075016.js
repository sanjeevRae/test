    ...
    if (organizationId) {
      try {
        ...
        if (!memberDoc.exists()) {
          ...
          console.log(`Existing member found, updating role from ${memberDoc.data().role} to ${role}`);
        }
        ...
        if (role === 'leader') {
          console.log(`Updating organization to include ${userId} as a leader`);
          ...
          if (orgDoc.exists()) {
            ...
            // Add the user to leaders array if not already there
            if (!leaders.includes(userId)) {
              await setDoc(orgRef, {
                leaders: [...leaders, userId],
                updatedAt: new Date().toISOString()
              }, { merge: true });
              console.log(`Added ${userId} to leaders list in organization document`);
            }
          }
        } else if (role === 'user') {
          // Handle demotion from leader to user by removing from leaders list
          console.log(`Checking if ${userId} needs to be removed from leaders list`);
          const orgRef = doc(db, 'organizations', organizationId);
          const orgDoc = await getDoc(orgRef);
          ...
          if (orgDoc.exists()) {
            ...
            // Remove the user from leaders array if present
            if (leaders.includes(userId)) {
              await setDoc(orgRef, {
                leaders: leaders.filter(id => id !== userId),
                updatedAt: new Date().toISOString()
              }, { merge: true });
              console.log(`Removed ${userId} from leaders list in organization document`);
            }
          }
        }
      } catch (orgError) {
...
export const refreshUserPermissions = async (userId = null) => {
  try {
    ...
    // If the user has an organization, check if they are a leader
    if (userData.organizationId) {
      ...
      if (orgDoc.exists()) {
        const orgData = orgDoc.data();
        const orgLeaders = orgData.leaders || [];
        ...
        if (orgLeaders.includes(userId) && userData.role !== 'leader') {
          ...
          await setDoc(userRef, { role: 'leader' }, { merge: true });
    ...
    if (userData.organizationId) {
      ...
      if (memberDoc.exists()) {
        ...
        if (memberData.role !== userData.role) {
          ...
          await setDoc(memberRef, { role: userData.role }, { merge: true });
        }
      }
    }
