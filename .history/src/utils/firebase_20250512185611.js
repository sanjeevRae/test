...
const cloudinaryStorage = {
  ...
  async uploadFile(file, path, options = {}) {
    ...
    try {
      ...
      // Call our Firebase Cloud Function instead of direct Cloudinary upload
      const uploadUrl = import.meta.env.VITE_FIREBASE_FUNCTIONS_URL || 'https://us-central1-evox-card.cloudfunctions.net';
      const response = await fetch(`${uploadUrl}/uploadToCloudinary`, {
        method: 'POST',
        body: formData,
        headers: {
      ...
      return {
        ...
        getDownloadURL: () => Promise.resolve(data.url),
        metadata: {
