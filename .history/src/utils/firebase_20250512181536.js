...
const cloudinaryStorage = {
  ...
  async uploadFile(file, path) {
    try {
      ...
      console.log('Starting Cloudinary upload to:', CLOUDINARY_UPLOAD_URL);
      
      // Upload to Cloudinary with improved CORS handling
      ...
      if (!response.ok) {
        ...
        throw new Error(`Cloudinary upload failed: ${response.statusText || 'Server error'}`);
      }
      
      const data = await response.json();
      console.log('Cloudinary upload successful, received data:', data);
      ...
      try {
        ...
        console.log('File reference saved to Firestore with ID:', fileDoc.id);
      } catch (firestoreError) {
        // Even if Firestore tracking fails, we still have the uploaded image URL
        console.error('Error saving file reference to Firestore:', firestoreError);
      }
      
      return {
        id: data.asset_id || `cloudinary-${Date.now()}`,
        fullPath: path,
        name: file.name,
