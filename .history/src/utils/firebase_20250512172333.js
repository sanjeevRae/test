...
// Network connectivity check with improved reliability
const checkNetworkConnectivity = () => {
  // Basic check using navigator.onLine
  const basicOnlineStatus = navigator.onLine;
  
  // If the basic check says we're offline, we definitely are
  if (!basicOnlineStatus) {
    return false;
  }
  
  // For more reliable results, we'll use the fact that we're already checking
  // Return true since the basic check passed - we'll verify actual connectivity
  // during the actual Firebase operations with proper error handling
  return true;
};
