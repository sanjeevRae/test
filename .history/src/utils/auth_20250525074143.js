...
export const addUserToOrganization = async (userId, organizationId, role = 'user') => {
  try {
    ...
    // Leaders can only manage existing users but cannot add new ones
    const canAddUser = currentUserRole === 'admin';
    
    if (!canAddUser) {
      console.error('You don\'t have permission to add users to this organization');
      alert('Permission denied: Only administrators can add users to organizations');
      return false;
    }
...
export const createUserInOrganization = async (userData, organizationId, role = 'user') => {
  try {
    ...
    if (currentUserRole !== 'admin') {
      console.error('Only administrators can create users');
      alert('Permission denied: Only administrators can create new users');
      return null;
    }
