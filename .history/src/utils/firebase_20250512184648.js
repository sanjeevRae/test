...
// Create a Cloudinary-based storage implementation with advanced features
const cloudinaryStorage = {
  ...
  async uploadFile(file, path, options = {}) {
    ...
    try {
      ...
      // For browser-based direct uploads - use upload preset instead of API secret
      // IMPORTANT: Make sure the upload preset is properly configured in Cloudinary dashboard
      // to allow unsigned uploads from your domain
      formData.append('upload_preset', 'ml_default');  // Replace with your upload preset that allows unsigned uploads
      
      // Add API key for proper identification
      ...
      // Add resource type - auto detect or specified
      formData.append('resource_type', 'auto');
      
      console.log('Uploading to Cloudinary URL:', CLOUDINARY_UPLOAD_URL);
      
      // Upload to Cloudinary with improved fetch options
      const response = await fetch(CLOUDINARY_UPLOAD_URL, {
        method: 'POST',
        body: formData,
        mode: 'cors',
        signal: controller.signal,
        // Do not set Content-Type header - browser will set it with proper boundary for FormData
      });
