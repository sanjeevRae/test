...
// Cloudinary upload function with secure server-side signing
exports.uploadToCloudinary = functions.https.onRequest((req, res) => {
  cors(req, res, async () => {
    try {
      ...
      // Process the upload when done parsing
      bb.on('finish', async () => {
        try {
          if (!fileBuffer) {
          ...
          const cloudName = functions.config().cloudinary?.cloud_name || process.env.CLOUDINARY_CLOUD_NAME;
          const apiKey = functions.config().cloudinary?.api_key || process.env.CLOUDINARY_API_KEY;
          ...
          const publicId = folderPath ? 
            `${folderPath}/${userId}_${timestamp}_${fileName.replace(/\.[^/.]+$/, "")}` : 
            `${userId}_${timestamp}_${fileName.replace(/\.[^/.]+$/, "")}`;
          ...
          // Generate signature (this is what makes it secure - done server-side)
          const params = {
            public_id: publicId,
          ...
          const signatureBaseArray = [];
          Object.keys(params).sort().forEach(key => {
            signatureBaseArray.push(`${key}=${params[key]}`);
          });
          
          // Add API secret
          signatureBaseArray.push(`api_secret=${apiSecret}`);
          const signatureBaseString = signatureBaseArray.join('&');
          
          // Generate SHA-1 hash of the signature base string
          const crypto = require('crypto');
          const signature = crypto.createHash('sha1').update(signatureBaseString).digest('hex');
          ...
          if (!response.ok) {
            ...
            res.status(response.status).send(`Cloudinary Error: ${errorText}`);
            return;
          }
      ...
      bb.on('error', (error) => {
        ...
        res.status(400).send(`Error parsing form: ${error.message}`);
      });
      ...
      console.error('Unhandled error in uploadToCloudinary:', error);
